...
def play(neural_controller):
    ...
    while not quit:
        ...
        if snake.movecount + MOVE_RATE > MOVE_THRESHOLD:
            days_passed += 1
            currentscore += 1  # score for staying alive?
            neural_controller.feed(snek=snake, apple=currentfood, board_size=SCREENTILES, tile_size=TILE_SIZE)
        ...
        if days_passed > STUCK_THRESHOLD:
            currentscore -= 2 * STUCK_THRESHOLD
            quit = True
            lose = True
        # score
